...
import { Users, FileText, Calendar, MessageSquare, UserCheck } from 'lucide-react';
...
export default function DashboardStats({ onNavigate }: DashboardStatsProps) {
  ...
  const loadStats = async () => {
    const [clients, documents, deadlines, meetings, accessRequests] = await Promise.all([
      ...
      supabase.from('access_requests').select('id', { count: 'exact', head: true }).eq('status', 'pending_approval'),
    ]);
    ...
    setStats({
      ...
      pendingAccessRequests: accessRequests.count || 0,
    });
  };
  ...
  const statCards = [
    ...
    { title: 'Pending Access Requests', value: stats.pendingAccessRequests, icon: UserCheck, color: 'text-amber-600', section: 'access-requests' },
  ];
